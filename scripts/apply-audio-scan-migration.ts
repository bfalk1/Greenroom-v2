/**
 * Applies the audio_scans migration to a target database.
 *
 * Same shape as scripts/apply-notification-migrations.ts and for the same
 ...
 *   npx tsx scripts/apply-audio-scan-migration.ts
 *
 * Then apply, naming the host you intend to hit:
 *   npx tsx scripts/apply-audio-scan-migration.ts --apply --confirm-host=aws-1-us-east-1.pooler.supabase.com
 */
...
const MIGRATION = "20260820000000_add_audio_scans";
...
async function main() {
  ...
  const present = await tableExists("audio_scans");
  ...
  if (!APPLY) {
    ...
    console.log(`  npx tsx scripts/apply-audio-scan-migration.ts --apply --confirm-host=${host}`);
  ...
  const ok = await tableExists("audio_scans");
  console.log(`\nVERIFY\n  ${ok ? "✓" : "✗"} audio_scans table exists`);
  await prisma.$disconnect();
